...
# Persist
regressor.save('saved_models/ftse-model-epoch-400-bs-64-do-0.2-un-40-lr-0.00005')

# regressor = km.load_model('model')

# Make predictions on test set
ftse_test = ut.read_csv('ftse_test.csv')
real_stock_prices = ftse_test.iloc[:,1:2].values

dataset_total = pd.concat((ftse_train['Open'], ftse_test['Open']), axis=0)
inputs = dataset_total[len(dataset_total) - len(ftse_test) - 50:].values
...
plt.plot(real_stock_prices, color='black', label='FTSE 100 Actual')
plt.plot(predicted_stock_price, color='green', label='FTSE 100 Predicted')
plt.title('FTSE 100 Predicted vs. Actual')
plt.xlabel('Time')
plt.ylabel('Price')
plt.legend()
plt.show()